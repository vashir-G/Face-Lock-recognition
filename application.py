...
def register():
    ...
    if request.method == "POST":
        ...
        # Query database to insert new user
            ...
            flash( "Successfully Registered As " + input_username)
...
def facereg():
    ...
    if request.method == "POST":
       ...
       # bill_face_encoding = face_recognition.face_encodings(image_of_bill)[0]
        encodings = face_recognition.face_encodings(image_of_bill)
        if len(encodings) > 0:
            bill_face_encoding = encodings[0]
        else:
...
def facesetup():
    if request.method == "POST":
        ...
        image_of_bill = face_recognition.load_image_file('./static/face/'+str(id_)+'.jpg')
        try:
           face_encodings = face_recognition.face_encodings(image_of_bill)
           if len(face_encodings) > 0:
               bill_face_encoding = face_encodings[0]
           else:
